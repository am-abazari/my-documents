...
دو قانون در رابطه با key ها داریم

1. کلید بین کامپوننت‌های هم لول (siblings) نباید یکسان باشه
2. کلید نباید با گذر زمان تغییر بکنه
...
const Parent = () => {
  return <Children increase={increaseFunction} setCount={setCount} />;
};
```
...
const Parent = () => {
  return (
    <Children
      ...
      Message={<p>successfully created OtherComponent</p>}
    />
  );
};
```
...
const Children = ({ counter }) => {
  return <ChildrensChildren counter={counter} />;
};
```
